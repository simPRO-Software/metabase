...
(defn- load-file-at-path [path]
  (slurp (or (io/resource path)
             (throw (Exception. (str "Cannot find '" path "'. Did you remember to build the Report Builder frontend?"))))))
...
(defroutes ^{:doc "Top-level ring routes for Metabase."} routes
  ...
  (context "/api" [] (fn [& args]
                       ...
                       (if-not (init-status/complete?)
                         {:status 503, :body "Report Builder is still initializing. Please sit tight..."}
